...
class SeleniumDrivenUserExpectations:
    ...
    @resetsLastVisitedLocator
    def withExactValue(self, expectedValue): 
    ...
    @requiresAPreviouslyVisitedLocator
    @resetsLastVisitedLocator
    def withExactText(self, expectedText):
    ...
    def withText(self, expectedText):
        currentText = self.getSeleniumInstance().get_text((self.seleniumExecutionContext.lastVisitedLocation))
        if expectedText not in currentText:
            raise SeleniumDrivenUserExpectationsException("Expected text : " + expectedText + " was not found in current text : " + currentText)
